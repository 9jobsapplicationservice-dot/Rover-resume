...
function scoreContactProfile({ resume }) {
  const checks = [
    [hasRealText(resume.name) && !isPlaceholder(resume.name), 3, "Add the candidate's full name."],
    [validPhone(resume.phone), 3, "Add a valid phone number."],
    [validEmail(resume.email), 3, "Add a valid email address."],
    [hasRealText(resume.location), 2, "Add city/state or location."],
    [hasRealText(resume.linkedin) && validUrlish(resume.linkedin), 2, "Add a valid LinkedIn profile URL."],
    [hasRealText(resume.github) && validUrlish(resume.github), 1, "Add a valid portfolio or GitHub URL when relevant."],
    [hasRealText(resume.targetRole), 1, "Add a target role for better ATS keyword alignment."],
  ];
...
function scoreAtsStructure({ resume }) {
  ...
  const checks = [
    [wordCount(resume.summary) >= 18 && wordCount(resume.summary) <= 90, 3, "Keep summary to 2-4 focused lines with role-specific value."],
    [hasExperience, 4, "Add at least one complete experience entry with bullets."],
    [hasProjects, 2, "Add projects or role-relevant work examples."],
    [splitSkills(resume.skills).length >= 8, 3, "Add at least 8 ATS-relevant skills."],
    [hasEducation, 2, "Add education details."],
    [(resume.certifications || []).length > 0 || hasRealText(resume.achievements), 1, "Add licences, certifications, or achievements if available."],
  ];
...
function scoreBulletQuality({ bullets }) {
  ...
  const checks = [
    [bullets.length >= 6, 4, "Add 6+ strong experience/project bullets."],
    [bullets.length >= 9, 3, "Add enough bullets to show depth across roles or projects."],
    [actionPercent >= 0.75, 4, "Start most bullets with strong action verbs."],
    [metricPercent >= 0.45, 5, "Add measurable outcomes, scale, time saved, volume, or frequency to more bullets."],
    [lengthPercent >= 0.7, 2, "Keep bullets concise: usually 8-32 words."],
