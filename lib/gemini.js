...
export async function optimizeWithGemini(rawText, targetRole, jobDescription, mode = "preserve") {
  ...
  const prompt = [
    ...
    preserveMode
      ...
      : "4. BULLET OPTIMIZATION: Improve the phrasing of existing bullets to be highly impactful ATS-friendly statements starting with powerful action verbs. Fix grammar and spelling, but retain the exact original meaning and facts.",
    "5. COMPLETE CONTENT: Do not truncate the summary or experience bullets. Extract only the Professional Summary/Profile text into resume.summary. Never include Employment History, Experience, Projects, Skills, Certifications, or Education content inside resume.summary.",
    "6. FORMATTING: Ensure the JSON structure perfectly reflects the candidate's history. Group skills accurately.",
    "7. Maintain the candidate's precise job titles, company names, and dates as written.",
    preserveMode
      ? "8. SECTION SAFETY: If the source has EMPLOYMENT HISTORY, WORK HISTORY, or CAREER HISTORY, put it in resume.experience. If the source has PROJECTS, every project must go into resume.projects, not resume.experience. If the source has EDUCATION, every institution/degree must go into resume.education. Do not merge Experience, Projects, Skills, Certifications, or Education into Summary."
      : "8. Tailor the summary and the first two bullets to the target role only when supported by the resume.",
    `Conversion mode: ${preserveMode ? "preserve uploaded resume data" : "enhance ATS wording"}`,
